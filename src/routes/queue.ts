...
const router = (fastify, { }, next) => {
  ...
  fastify.post('/prepare/register', { preHandler: [fastify.authenticate] }, async (req: fastify.Request, reply: fastify.Reply) => {
    ...
    var sHn : any = await queueModel.searchHN(db,dateServ,hn);
    //fastify.log.error(sHn[0].cc);
    if (hn && sHn[0].cc =="0") {
      // get patient info
<<<<<<< HEAD
...
>>>>>>> first commit
      ...
      if (rsp.length) {
        ...
        var qHis : any = await hisModel.getHISQueue(dbHIS,hn,dateServ);
        const hisQueue = qHis[0].queue;
...
=======
              ...
              var useHISQueue = process.env.USE_HIS_QUEUE || 'Y';
              var _queueRunning = 0;
              if(useHISQueue === 'Y'){
...
>>>>>>> first commit
  ...
  fastify.get('/waitQ/:servicePointId', { preHandler: [fastify.authenticate] }, async (req: fastify.Request, reply: fastify.Reply) => {
    ...
    try {
      const dateServ: any = moment().format('YYYY-MM-DD');
      const rsF: any = await queueModel.getWorking(db, dateServ, servicePointId, query);
      var currentQueue = 1;
      if(rsF.length > 0){
        currentQueue = rsF[0].queue_number;
      }
      const rs: any = await hisModel.getHISWaitQueue(dbHIS, dateServ, servicePointId,currentQueue, query);
      reply.status(HttpStatus.OK).send({ statusCode: HttpStatus.OK, results: rs });
    } catch (error) {
      fastify.log.error(error);
      reply.status(HttpStatus.INTERNAL_SERVER_ERROR).send({ statusCode: HttpStatus.INTERNAL_SERVER_ERROR, message: HttpStatus.getStatusText(HttpStatus.INTERNAL_SERVER_ERROR) })
  ...
  fastify.post('/working/history/:servicePointId', { preHandler: [fastify.authenticate] }, async (req: fastify.Request, reply: fastify.Reply) => {
