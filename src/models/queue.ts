...
export class QueueModel {
  ...
  searchHN(db: knex, dateServ: any, hn: any) {

    return db('q4u_queue as q')
      .select(db.raw('count(hn) as cc'))
      .where('q.hn', hn)
      .where('q.date_serv', dateServ);
  ...
  getWaitingList(db: knex, dateServ: any, servicePointId: any, limit: any, offset: any, sort: any = '', query = '') {
    ...
    const sql = db('q4u_queue as q')
      ...
      .where('q.is_completed', 'N')
      .where((w) => {
        w.orWhereRaw(`REPLACE(q.queue_number,' ','') like '${_query}'`);
      })
      .whereNot('q.is_cancel', 'Y')
  ...
  getWorking(db: knex, dateServ: any, servicePointId: any, query: any) {
    ...
    const sql = db('q4u_queue_detail as qd')
      .select('qd.service_point_id', 'q.queue_interview', 'qd.date_serv as queue_date', 'qd.last_queue', 'qd.room_id',
        ...
        'r.room_name', 'r.room_number', 'sp.service_point_name', 'sp.department_id', 'q.is_completed', db.raw('SUBSTRING_INDEX(room_name," ",1) as splittedRoomName '))
...
>>>>>>> first commit
  ...
  getQueueWait(db: knex, dateServ: any, servicePointId: any, query: any) {
    const sql = `SELECT q.queue_number,(SELECT time_avg from q4u_service_points WHERE service_point_id=q.service_point_id) as timeavg from q4u_queue q 
    WHERE q.date_serv =? and q.is_completed <> 'Y'
    and q.service_point_id =?
     and q.queue_number > (select q.queue_number from q4u_queue_detail qd
          INNER JOIN q4u_queue q on q.queue_id = qd.queue_id 
          where qd.date_serv=?
          and qd.service_point_id =?
          and q.mark_pending <> 'Y'
          and q.is_cancel <> 'Y'
          GROUP BY qd.date_serv, qd.service_point_id, qd.room_id ORDER BY qd.update_date desc limit 1)  
    ORDER BY queue_number limit 7
    `;
    return db.raw(sql, [dateServ,servicePointId, dateServ,servicePointId]);
  }
  ...
  getWorkingHistoryGroup(db: knex, dateServ: any, servicePointId: any) {
    const sql = db('q4u_queue as q')
      .select('q.service_point_id', 'q.date_serv as queue_date', 'qgd.room_id',
        'q.queue_number', 'q.queue_running', 'q.hn', 'q.vn', 'q.queue_id', 'q.queue_interview', 'q.date_serv', 'q.time_serv', 'q.date_update', 'p.title', 'p.first_name', 'p.last_name',
        'p.birthdate', 'pr.priority_name', 'pr.priority_id', 'pr.priority_color',
        'r.room_name', 'r.room_number', 'sp.service_point_name', db.raw('SUBSTRING_INDEX(room_name," ",1) as splittedRoomName '))
      // .innerJoin('q4u_queue as q', 'q.queue_id', 'qd.queue_id')
      .innerJoin('q4u_person as p', 'p.hn', 'q.hn')
      .innerJoin('q4u_priorities as pr', 'pr.priority_id', 'q.priority_id')
      .innerJoin('q4u_queue_group_detail as qgd', 'qgd.queue_id', 'q.queue_id')
